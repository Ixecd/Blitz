...
```
namespace: blitz
  blitz-controller（Deployment）  1/1 Running ✅
  wallet-service（Deployment ×2）       1/1 Running ✅  v0.1.12
  blitz-etcd-0（StatefulSet）      1/1 Running ✅  PVC
  blitz-postgres-0（StatefulSet）  1/1 Running ✅  PVC
```
...
├── deployments/blitz/
│   ├── blitz-postgres/   StatefulSet + PVC
│   ├── blitz-etcd/       StatefulSet + PVC
│   ├── wallet-service/        secretKeyRef 注入敏感变量
│   └── blitz-controller/ 最小权限 RBAC
