...
**根因**：`dtk deploy` 跑 `helm upgrade --install web3-blitz`，这是另一个 release，不会动 postgres release。但 postgres 消失的原因是 Bitnami 限制，之前的 release 因为 PVC 密码不匹配问题被手动删除过。  
**解决**：把 postgres 纳入 web3-blitz chart 的 templates，不再依赖独立 release。  
...
```
新增：
- deployments/web3-blitz/templates/postgres-statefulset.yaml
- deployments/web3-blitz/templates/etcd-deployment.yaml

修改：
- deployments/web3-blitz/templates/deployment.yaml     ← 加 env/envFrom + initContainers
- deployments/web3-blitz/templates/wallet-service-deployment.yaml ← initContainers 改 service name
- deployments/web3-blitz/Chart.yaml                    ← dependencies: []
- deployments/web3-blitz/values.yaml                   ← DATABASE_URL=postgres:5432, ETCD_ENDPOINTS=etcd:2379
- internal/db/migrations/                              ← 删除重复的 000002_add_email_to_users

删除：
- deployments/web3-blitz/templates/deployment.yaml（nginx 占位）
- deployments/web3-blitz/charts/*.tgz
...
```
snapshots/
├── SNAPSHOT-web3-blitz-2026-03-23-auth-migrate.md
├── SNAPSHOT-web3-blitz-2026-03-23-k8s-deploy.md
└── SNAPSHOT-web3-blitz-2026-03-24-fully-deployed.md   ← 本次 🏆
```
