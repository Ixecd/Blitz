...
验证：`blitz-etcd-0` 挂载 PVC `etcd-data-blitz-etcd-0` ✅
...
- deployments/blitz/blitz-etcd/templates/deployment.yaml → statefulset.yaml
- deployments/blitz/wallet-service/templates/deployment.yaml  ← secretKeyRef
- deployments/blitz/wallet-service/values.yaml                ← 删除敏感 env
...
```
namespace: blitz
  blitz-controller-xxx    1/1 Running  ✅
  wallet-service-xxx (×2)       1/1 Running  ✅
  blitz-etcd-0             1/1 Running  ✅  StatefulSet + PVC
  blitz-postgres-0         1/1 Running  ✅  StatefulSet + PVC
```
...
└── SNAPSHOT-blitz-2026-03-30-k8s-hardening.md  ← 本次
```
